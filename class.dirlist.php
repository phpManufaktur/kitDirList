...
class kitDirList {
  ...
  public function __construct($silent = true) {
    ...
    if (!isset($_SESSION[self::session_prefix.self::session_protect]))
      $_SESSION[self::session_prefix.self::session_protect] = self::protect_undefined;
    if (!isset($_SESSION[self::session_prefix.self::session_user]))
      $_SESSION[self::session_prefix.self::session_user] = '';
    if (!isset($_SESSION[self::session_prefix.self::session_auth]))
    ...
    foreach ($this->params as $key => $value) {
      switch ($key) :
        ...
        case self::param_sort:
          $this->params[$key] = self::sort_asc;
          break;
        case self::param_login_dlg:
          $this->params[$key] = 'kit_login';
          break;
        case self::param_account_dlg:
          $this->params[$key] = 'kit_account';
          break;
        default:
  ...
  private function checkProtection() {
    if (strpos($this->base_path, $this->protected_path) !== false) {
      ...
      if (!empty($_SESSION[self::session_prefix.self::param_kit_news]) || !empty($_SESSION[self::session_prefix.self::param_kit_dist]) || !empty($_SESSION[self::session_prefix.self::param_kit_intern])) {
  ...
  private function checkPaths() {
    ...
    if (!file_exists($this->contacts_path)) {
      if ($this->kit_installed) {
        ...
        global $dbRegister;
        $SQL = sprintf("SELECT * FROM %s WHERE %s='%s'", $dbRegister->getTableName(), dbKITregister::field_status, dbKITregister::status_active);
        $registers = array();
    ...
    if (empty($this->base_path))
      $this->base_path = $this->media_path.$_SESSION[self::session_prefix.self::param_media];
    // check base path
  ...
  public function generatePassword($length = 7) {
    ...
    $not = array_merge(array('i', 'l', 'o', 'I', 'O'), $this->old_pass);
    $r = array_diff($r, $not);
    shuffle($r);
  ...
  private function getFileDescriptions($directory) {
    ...
    if (!file_exists($desc_file))
      return false;
    if (false === ($descriptions = file($desc_file, FILE_IGNORE_NEW_LINES | FILE_SKIP_EMPTY_LINES)))
      return false;
  ...
  public function directoryListing() {
    ...
    if ((isset($_SESSION[self::session_prefix.self::param_mkdir]) && ($_SESSION[self::session_prefix.self::param_mkdir] == true)) && (!isset($_SESSION[self::session_prefix.self::param_recursive]) || (isset($_SESSION[self::session_prefix.self::param_recursive]) && ($_SESSION[self::session_prefix.self::param_recursive] == false)))) {
    ...
    if (($_SESSION[self::session_prefix.self::session_protect] == self::protect_group) || ($_SESSION[self::session_prefix.self::session_protect] == self::protect_kit) || ($_SESSION[self::session_prefix.self::session_protect] == self::protect_wb)) {
      $redirect = true;
    ...
    if (isset($_REQUEST[self::request_action]) && ($_REQUEST[self::request_action] == self::action_upload)) {
      ...
      if (isset($_FILES[self::request_file]) && (is_uploaded_file($_FILES[self::request_file]['tmp_name']))) {
        if ($_FILES[self::request_file]['error'] == UPLOAD_ERR_OK) {
          ...
          $ext = end($explode);
          if ((in_array($_FILES[self::request_file]['tmp_name'], $this->general_excluded_files)) || (in_array($ext, $this->general_excluded_extensions))) {
    ...
    foreach ($directory as $item) {
      ...
      if ($item == '..') {
        ...
        if (in_array(strtolower($item), $this->general_excluded_files))
          continue;
        // check for general excluded extensions...
        if (in_array(strtolower($file_info['extension']), $this->general_excluded_extensions))
          continue;
        if (!empty($_SESSION[self::session_prefix.self::param_include])) {
          ...
          if (!in_array(strtolower($file_info['extension']), $include))
            continue;
        }
        ...
        $file = sprintf('<a href="%s" %starget="_blank">%s %s</a>', $file_link, $desc, sprintf('<img src="%s" width="16" height="16" alt="%s" />', $this->icon_url.$mimeType->getIconByType($dir.$item), $mimeType->getMimeType($item)), $item);
      } else {
        // Verzeichnis...
        $unlink_type = 'dir';
        if (($redirect == false) && ($item == self::protection_folder))
          continue;
        $size = '';
        $date = '';
      ...
      if ($this->params[self::param_unlink] && ($unlink_type !== 'none')) {
        ...
        $params = sprintf('%s%s=%s&%s=%s&%s=%s', ($is_sub_dir) ? sprintf('%s=%s&', self::request_sub_dir, $sub_dir) : '', self::request_unlink, ($unlink_type == 'dir') ? self::action_unlink_dir : self::action_unlink_file, self::request_unlink_confirm, self::action_unlink_pending, self::request_unlink_item, $item);
        $unlink = sprintf('<a href="%s%s%s"><img src="%s" width="16" height="16" alt="%s" /></a>', $this->page_link, (strpos($this->page_link, '?') === false) ? '?' : '&', $params, $this->icon_url.'unlink.gif', kdl_alt_unlink);
      } else {
        $unlink = '';
      }
